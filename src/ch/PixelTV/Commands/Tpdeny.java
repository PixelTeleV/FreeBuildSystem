    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if(args.length == 1) {
            if(target == null) {
                ...
                if(target == p) {
                    p.sendMessage(Main.pr + "Du §ckannst nicht §7deine eigene §eTPA-Anfrage §cablehnen§7!");
                } else {
                    if(Tpa.requests.containsKey(p)) {
                        if(Tpa.requests.get(target).contains(p)) {
                            Tpa.requests.get(target).remove(p);
                            p.sendMessage(Main.pr + "Der Spieler §e" + target.getName() + " §7hat deine §eTPA-Anfrage §cabgelehnt§7!");
                            target.sendMessage(Main.pr + "Du hast die §eTPA-Anfrage §7von dem Spieler §e" + p.getName() + " §cabgelehnt§7!");
                        }
                    } else {
